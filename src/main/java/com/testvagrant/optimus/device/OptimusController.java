...
import com.testvagrant.optimus.utils.CurrentTime;
import com.testvagrant.optimus.utils.OnDevice;
import com.testvagrant.optimus.utils.WDAServerManager;
...
//            }
    ...
    public List<SmartBOT> registerSmartBOTs() throws IOException, InterruptedException, DeviceMatchingException, DeviceEngagedException {
        ...
        for (Map.Entry<String, DesiredCapabilities> entry : appToDeviceMap.entrySet()) {
            ...
            if(platformIOS(entry.getValue())) {
                ...
                WDAServerManager serverManager = new WDAServerManager(udid);
                serverManager.startServer();
                String serverURL = serverManager.getServerUrl();
                int port = serverManager.getPort();
                appiumService = new AppiumServerManager(optimusConfigParser.getExecutionDetails())
                        .withPort(port)
                        .startAppiumService(uniqueScenarioName,udid);
                driver = addDriver(appiumService.getUrl(), entry.getValue(),serverURL);
            } else {
                appiumService = new AppiumServerManager(optimusConfigParser.getExecutionDetails())
                        .startAppiumService(uniqueScenarioName, udid);
    ...
    public AppiumDriver addDriver(URL url, DesiredCapabilities capabilities, String wdaServerURL) {
        try {
            capabilities.setCapability("webDriverAgentUrl",wdaServerURL);
            return setUpDevice(url, capabilities);
        } catch (MalformedURLException e) {
            e.printStackTrace();
        }
